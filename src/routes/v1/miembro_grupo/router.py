...
async def get_miembro_grupo(
  request: Request,
  id: int
) -> MiembroGrupoResponseSchema:
  data_usuario = request.state.data_usuario
  usuario_id = data_usuario.get("sub")

  db = Session()
  result = MiembroGrupoService(db).get_miembro_grupo(id, usuario_id)
  if not result:
    raise HTTPException(status_code=HTTP_404_NOT_FOUND, detail={'message': 'No se encontró el miembro del grupo'})
  return result
...
async def aceptar_invitacion_miembro_grupo(
  request: Request,
  token_data: TokenAceptarInvSchema
) -> MiembroGrupoSchema:
  data_usuario = request.state.data_usuario
  usuario_id = data_usuario.get("sub")

  db = Session()
  result = MiembroGrupoService(db).aceptar_invitacion_miembro_grupo(token_data, usuario_id)
  if not result:
